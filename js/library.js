...
function renderLibrary() {
    ...
    if (topics.length === 0) {
        container.innerHTML = createEmptyLibraryHTML();
        return;
    }
    ...
    container.innerHTML = topics.map((topic, index) => {
        ...
        return createTopicCardHTML(topic, count, iconColor);
    }).join('');
}

// Render danh sách từ vựng chi tiết
function renderVocabList(resetLimit = true) {
    ...
    let htmlStr = itemsToRender.map((item) => {
        ...
        return createVocabCardHTML(item, originalIndex);
    }).join('');
...
// Tạo giao diện thư viện trống
function createEmptyLibraryHTML() {
    return `
...
// Tạo thẻ chủ đề
function createTopicCardHTML(topic, count, iconColor) {
    return `
...
// Tạo thẻ từ vựng chi tiết
function createVocabCardHTML(item, originalIndex) {
    ...
    return `
        <div class="bg-white dark:bg-slate-800 p-5 md:p-6 rounded-[2rem] border border-slate-100 dark:border-slate-700 shadow-sm hover:shadow-lg transition-all duration-300 group relative overflow-hidden flex flex-col">
            <div class="absolute top-5 right-5 flex gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300 z-10 transform translate-x-4 group-hover:translate-x-0">
                <button onclick="speak('${escapedWord}')" class="w-9 h-9 flex items-center justify-center bg-blue-50 hover:bg-blue-500 text-blue-500 hover:text-white dark:bg-blue-900/30 rounded-xl transition-colors shadow-sm">🔊</button>
