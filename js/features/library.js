...
const LibraryModule = (function() {
    ...
    function renderLibrary() {
        ...
        if (topics.length === 0) {
            ...
            <div class="col-span-full py-16 text-center bg-white dark:bg-slate-800 rounded-[2rem] border border-dashed border-slate-200 dark:border-slate-700">
                ...
                <button onclick="appEventBus.emit('TAB_CHANGED', 'ai-gen')" class="mt-6 bg-blue-50 dark:bg-blue-900/30 text-blue-600 px-6 py-3 rounded-xl font-black text-xs uppercase tracking-widest hover:bg-blue-100 transition-colors">Tạo từ mới ✨</button>
            </div>`;
            return;
        }
        ...
        container.innerHTML = topics.map((topic, index) => {
            ...
            return `
            <div class="group relative overflow-hidden bg-white dark:bg-slate-800 p-5 lg:p-6 rounded-[2rem] border border-slate-100 dark:border-slate-700 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 cursor-pointer" data-action="start-flashcards" data-payload="${topic}">
    ...
    function renderVocabList(resetLimit = true) {
        ...
        let htmlStr = itemsToRender.map((item) => {
            ...
            const hasSynonyms = item.synonyms && item.synonyms !== 'N/A' && item.synonyms !== 'None';
            ...
            <div class="bg-white dark:bg-slate-800 p-5 md:p-6 rounded-[2rem] border border-slate-100 dark:border-slate-700 shadow-sm hover:shadow-lg transition-all duration-300 group relative overflow-hidden flex flex-col">
                <div class="absolute top-5 right-5 flex gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300 z-10 transform translate-x-4 group-hover:translate-x-0">
                    <button data-action="speak" data-payload="${safeWord}" class="w-9 h-9 flex items-center justify-center bg-blue-50 hover:bg-blue-500 text-blue-500 hover:text-white dark:bg-blue-900/30 rounded-xl transition-colors shadow-sm">🔊</button>
    ...
    // Tự động render lại khi có event cập nhật từ vựng
...
window.loadMoreVocab = LibraryModule.loadMoreVocab;
